...
- **Live assertions (run against `komun_a`, on port 3011 so you don't collide with anything):** **11 rapid signins from one IP → 429** — measured: `RouteClass::SignIn` is `Quota::new(10, 300s)`, so the first refusal is attempt 11 (observed `401 x10, 429 x4` with a different forged `X-Forwarded-For` on every attempt, all sharing one bucket). The number originally written here was WRONG; the policy was not to be changed to match it, and the property that matters — bounded guesses per IP, a forged forwarded address buys nothing — holds; a spoofed `X-Forwarded-For` from an untrusted peer ignored; forgot for an unknown email → 200 and sends nothing; a used/expired reset token rejected; reset **with** the recovery code makes a pre-reset message readable and **without** it does not; every session gone after a password reset; change-password keeps the same x25519 secret; reissuing a recovery code invalidates the previous one; the recovery code is never returned by any endpoint after signup.
...
## Post-card record (orchestrator, after A2b reported)

**Accepted as correct — three findings from the worker:**
1. The rate-limit number above was the orchestrator's error; the worker reported the measured threshold instead of changing the product policy to fit the card. Keep that behaviour.
2. `PATCH /admin/users/{id}/role` was repaired rather than left alone. It predates A2b, but this card scopes role change to superadmin-only with an `audit_events` row, and the route did neither: no audit row, and its refusals returned HTTP 200 bodies with an `error` key, so a client reading the status code saw a rejected change as applied. Now 400/404, auditing every change including no-ops.
3. `GET /auth/password-reset/bundle?token=…` was added even though this card did not list it. Without it the "reset with the recovery code" assertion was unimplementable: only `/auth/signin` returned key material and someone resetting a forgotten password cannot sign in. It looks the token up without consuming it, is rate-limited under `PasswordReset`, and returns ciphertext only.

**Applied by the orchestrator from `hub-requests/A2b.md`:** all 8 sections across `web/src/routes/+layout.svelte` (3) and `web/src/routes/connect/+page.svelte` (5), plus the deletion of `web/src/lib/components/Onboarding.svelte`. `grep -rn passphrase web/src | grep -v test` is now EMPTY, so that gate is reachable.

**Environment the orchestrator supplied (this card's BLOCKED items 1 and 3):** `web/node_modules` installed and `crates/wasm/pkg` rebuilt over a bridge-networked throwaway. Measured on the real tree: `npm run check` 14 errors / 37 warnings in 8 files (exactly the frozen baseline — no new diagnostic), `npm run build` green, `npx vitest run` 1 failed | 43 passed (44) — baseline was 43 total, so this card added one passing test and introduced no new failure.

**Left for the next A card:** remove the four shims (`register`, `recover`, `showOnboarding`, `onAuthComplete`) from `web/src/lib/stores/auth.ts` — nothing imports them now that the hub diffs landed. Known cosmetic nit, not to be fixed silently: `.join-link` was styled as a `<button>` and is now an `<a>`, so it may pick up the nav underline (`text-decoration: none` / `display: inline-block`). Visual change — surface it to the user rather than deciding it.
