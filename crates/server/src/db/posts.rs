...
use komun_core::models::{CreatePost, Post, PostKind, PostStatus, Urgency, Visibility};
...
pub async fn list(
    ...
    let rows = sqlx::query_as::<_, PostRow>(
        r#"SELECT id, author_id, kind, category, title, body,
...
pub async fn create(
    ...
    let visibility = match input.visibility.unwrap_or(Visibility::Public) {
...
impl From<PostRow> for Post {
    fn from(r: PostRow) -> Self {
        Post {
            ...
            // marketplace facet: not selected by this query yet — A3.1 writes the real one
            market_listed: false,
            price_cents: None,
            currency: None,
