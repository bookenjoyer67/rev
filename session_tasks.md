...
`.dispatch/SPEC.md`. Agents never run git mutations; the orchestrator commits, merges and rebases.
...
- `migrations/**`
- `crates/core/src/**`
- `crates/relay/**` (delete only)
- `crates/server/src/relay_bridge.rs`, `crates/server/src/relay_ops.rs` (delete only)
...
- `psql "$DATABASE_URL"` — its own DB only (`komun_a` on `komun-db-a`); read/write schema probes and fixtures
- `cd web && npm run check && npm run build && npx vitest run`, `node`
- `grep`, `rg`, `find`, `git status`, `git diff`, `git log` (read-only git)
- NOT permitted: any git mutation (commit, push, checkout, restore, stash, rebase, reset, worktree), `cargo add` / `npm install` / any network fetch, `docker`, `sudo`, writes outside `/workspace`, and any write to Session B's DB

Definition of done:
- A1: `cargo test -p komun-core` green, `cargo build --workspace` clean, `grep -ri piggpin .` empty, `psql -c "\dt"` lists exactly the schema in SPEC Part 1.4, every negative insert rejected (negative price, `market_listed` on a `need`, lowercase currency, `item_condition='mint'`, rating 6, duplicate review, offer kind `'bid'`, `role='root'`, `visibility='federated'`, case-differing duplicate email, unknown category slug, `DELETE` of a category in use), and the enum↔CHECK test provably fails when one CHECK entry is deleted by hand
...
- Every change is inside the write list above, hub requests from Session B are applied and the build is re-verified after each, and the final message reports `FILES WRITTEN`, `GATES RUN` with raw output, and `BLOCKED`
...
- deletions: `crates/server/src/api/alliances.rs`, `crates/server/src/db/alliances/**`, `crates/server/src/federation/**`
- `.dispatch/hub-requests/**` (its only permitted way to request changes to files it does not own)

Files or folders the agent may read but not write to:
- `crates/server/src/api/mod.rs`, `crates/server/src/config.rs`, `crates/server/src/main.rs`, `crates/server/src/auth/**`, `crates/server/src/db/**` except the deletion paths above, `crates/server/src/tasks/**`
...
- `psql "$DATABASE_URL"` — its own DB only (`komun_b` on `komun-db-b`); no `DELETE`/`TRUNCATE`/`DROP`
- `cd web && npm run check && npm run build && npx vitest run`, `node`
- `grep`, `rg`, `find`, `git status`, `git diff`, `git log` (read-only git)
- NOT permitted: any git mutation (commit, push, checkout, restore, stash, rebase, reset, worktree), `cargo add` / `npm install` / any network fetch, `docker`, `sudo`, writes outside `/workspace`, and every file in the read-only list above (a hub request is the substitute)
...
- Every change is inside the write list above; anything else was requested as a hub request rather than edited; the final message reports `FILES WRITTEN`, `GATES RUN` with raw output, and `BLOCKED`
